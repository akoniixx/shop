          ...
          <View style={{
            ...
            <View >
              <View style={{ paddingHorizontal: 20, paddingVertical: 20, flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text fontSize={18} fontFamily='NotoSans' semiBold>สินค้าที่ยังไม่ได้นำขึ้นรถ</Text>
                <Text color='secondary' >{currentList.filter(item => item.quantity > 0).length}/{currentList.length} รายการ</Text>
              </View>
              <DashedLine
                dashGap={0}
                dashThickness={0.5}
              ...
              <ScrollView  style={{ maxHeight: "70%"}} >
                ...
                {currentList.filter(item => item.quantity > 0).map((item) => (
